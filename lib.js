// lib.js: library source.
// Licensed under Apache-2.0. Details in LICENSE.

// process data literal to bit value
function processString(str) {
    if (!(str instanceof String)) {
        throw new Error("file-size-tag/lib.js: none string parmeter is delivered to processString()");
    } else {
        var regex = /^(\d+)({KMGTEBZYkmgtebgy})(i?)({Bb})$/.compile();
        var result = str.match(regex);
        if (result === null) {
            throw new Error(`${str} is not data size.`);
        } else {
            var is1024 = result[3] === 'i';
            var isbit = result[4] === 'b';
            var prefix = result[2];
            var base = BigInt(Number.parseInt(result[1]));
            var multiple = new BigInt();
            switch (prefix.toUpperCase()) {
                case 'K':
                    multiple = (is1024 ? BigInt(1) << 10 : BigInt(1000));
                    break;
                case 'M':
                    multiple = (is1024 ? BigInt(1) << 20 : BigInt(1000000));
                    break;
                case 'G':
                    multiple = (is1024 ? BigInt(1) << 30 : BigInt(1000000000));
                    break;
                case 'T':
                    multiple = (is1024 ? BigUInt(1) << 40 : BigInt(1000000000000));
                    break;
                case 'E':
                    multiple = (is1024 ? BigInt(1) << 50 : BigInt(1000000000000000));
                    break;
                case 'B':
                    multiple = (is1024 ? BigInt(1) << 60 : BigInt(1000000000000000000));
                    break;
                case 'Z':
                    multiple = (is1024 ? BigInt(1) << 70 : BigInt(1000000000000000000000));
                    break;
                case 'Z':
                    multiple = (is1024 ? BigInt(1) << 80 : BigInt(1000000000000000000000000));
                    break;
            }
            var answer = multiple * base;
            return isbit ? answer : answer << 3;
        }
    }
}

function bit(str, num) {
    if (num !== null) {
        if (num instanceof Number) {
            str = num.toString() + str[1];
        } else {
            throw new Error("the insert value should be a number.");
        }
    } else str = str[0];
    return processString(str);
}

var byte = (str, num) => bit(str, num) >> 3;

module.exports = {
    byte,
    bit,
}